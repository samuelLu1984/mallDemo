        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(address,index) in addrList" :class="{'check':checkIndex==index}" @click="checkedAddressId=address.addressId;checkIndex=index">
                    <dl>
                      <dt>{{address.postName}}</dt>
                      <dd class="address">{{address.postAddress}}</dd>
                      <dd class="tel">{{address.postTel}}</dd>
                    </dl>
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click="delAddressForm(address.addressId)">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration addr-set-default" v-if="!address.isDefault">
                      <a href="javascript:;" class="addr-set-default-btn"  @click="setDefault(address.addressId)"><i>Set default</i></a>
                    </div>
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red " :to="{name:'order',query:{id:checkedAddressId}}">Next</router-link>
            </div>
          </div>
        </div>
        <modal :md-flag="addressModalFlag" @close="closeModal">
          <div slot="md-title" class="md-title">增加收货地址</div>
          <div slot="modal-msg" >
            ...
            <ul>
              <li class="regi_form_input">
                ...
                <input type="text" tabindex="1" name="postName" v-model="postName" class="regi_login_input regi_login_input_left" placeholder="请输入收件人" >
              </li>
              <li class="regi_form_input">
                <i class="icon IconPeople"></i>
                <input type="text" tabindex="2" name="postAddress" v-model="postAddress" class="regi_login_input regi_login_input_left" placeholder="请输入您的收货地址" >
              </li>

              <li class="regi_form_input noMargin">
                <i class="icon IconPwd"></i>
                <input type="text" tabindex="3"  name="postTel" v-model="postTel" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="输入手机或者电话" >
              </li>
              <li class="">
                <i class="icon IconPwd"></i>
                <input type="checkbox" tabindex="4"  name="isDefault" v-model="isDefault" class="" placeholder="是否设为默认" @keyup.enter="">是否设为默认
              </li>
            </ul>
          </div>
          <a href="javascript:;" class="btn-login" @click="newAddress" slot="modal-btn">添加</a>
        </modal>
        <modal :md-flag="delFlag" @close="closeModal">
          <p slot="modal-msg" >

            <span class="error-msg">您确定要删除吗</span>
          </p>
          <div slot="modal-btn">
            <a href="javascript:;"   class=" btn btn-login" @click="delAddress()">确定</a>
            <div style="height: 30px"></div>
            <a href="javascript:;"   class=" btn btn-login btn--red" @click="delFlag=false">取消</a>
          </div>
        </modal>
      </div>
...
<script>
     ...
     export default {
       ...
       methods:{
         ...
         newAddress(){
           if (!this.postName||!this.postAddress||!this.postTel) {
             return this.errorTip='收件人、收货地址和电话不能为空'
           }
         ...
         setDefault(index){
           axios.post('/user/edit/setDefaultAddress',{
             userId:sessionStorage.userId,
             addressId:index
           }).then(()=>{
             this.getAddressList()
         ...
         delAddressForm(index){
           this.delFlag=true
           this.delAddressId=index
         },
         delAddress(){
            axios.post('/user/edit/delAddress',{
              userId:sessionStorage.userId,
         ...
         // setOrder(){
         //   if (sessionStorage.formId==1){
         //     return
         //   }
         //   sessionStorage.formId=1
         //   axios.post('/user/setOrder',{
         //     userId:sessionStorage.userId,
         //     addressId:this.checkedAddressId
         //   }).then(res=>{
         //
